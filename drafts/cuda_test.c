...
int main()
{
	...
	int i;
	char name[100];
	int cc_major, cc_minor, cores, cuda_cores, threads_per_core, clockrate, cuda_driver_version, cuda_runtime_version;
	size_t freeMem;
	size_t totalMem;
  ...
  result = cudaDriverGetVersion(&cuda_driver_version);
  if (result == CUDA_SUCCESS) {
    printf("Cuda Driver Version: %d.%d\n", cuda_driver_version/1000, cuda_driver_version%100 / 10);
  }

  result = cudaRuntimeGetVersion(&cuda_runtime_version);
  if (result == CUDA_SUCCESS) {
